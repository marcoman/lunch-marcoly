// Console grid navigator — guarded rollout a single LaunchDarkly highlight flag.
...
constexpr const char* kAppBanner = "15-guarded-rollout[cpp]";
...
void render(const std::string& username, int row, int col,
    ...
    const std::string name_line =
        ...
        colorize(" " + flags.colorLabel, flags.highlightColor) + kReset + kBgReset;
    write_line(kAppBanner);
    write_line(name_line);
    write_line("Flag value: " + flags.flagValue);
    ...
    write_line("Toggle the flag in LaunchDarkly — changes appear within ~1s.");
    write_line("");
