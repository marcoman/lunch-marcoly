#!/usr/bin/env bash
# Shared REST helpers for 15-guarded-rollout.
...
fetch_flag_with_guarded_rollout() {
  api GET "/flags/${LD_PROJECT_KEY}/${FLAG_KEY}?env=${LD_ENVIRONMENT_KEY}&expand=guardedRollout" \
    -H "LD-API-Version: ${BETA_API_VERSION}"
}
