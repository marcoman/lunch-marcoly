...
/// <summary>
/// Fetch recent Yahoo Finance news titles for tickers (no API key, no LaunchDarkly).
...
public static class YahooNews
{
    ...
    /// <summary>
    /// Locate the 01-reference-agent example root (the folder containing
    /// <c>prompts/</c> and <c>stories/</c>) so the system prompt and cache resolve the
    /// same way whether you run <c>dotnet run</c> from <c>dotnet/</c> or launch the
    /// published binary directly.
    /// </summary>
    public static string ExampleRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(candidate, "prompts", "system_prompt.txt")))
            {
