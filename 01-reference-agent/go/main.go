// 01-reference-agent[go] — terminal UI matching the Python / Node / Java consoles.
package main
...
const (
	appBanner  = "01-reference-agent[go]"
	chromeRows = 3
	footerRows = 1
	padMax     = 4000
...
var llmModes = []string{"stub", "ollama", "bedrock"}
...
func (a *app) render() {
	...
	left1 := fmt.Sprintf("AGENT_LLM_MODE=%s  model=%s", mode, model)
...
func (a *app) cmdStatus() {
	...
	a.append(fmt.Sprintf("Provider: %s / %s", mode, modelLabel(mode)), "muted")
...
func (a *app) cmdMode() {
	current := resolveMode()
	idx := 0
	for i, m := range llmModes {
		if m == current {
			idx = i
			break
		}
	}
	nxt := llmModes[(idx+1)%len(llmModes)]
	if nxt == "ollama" && !probeOllama(600*time.Millisecond) {
		a.append(fmt.Sprintf("Ollama not reachable at %s. Start Ollama and pull a model.", ollamaHost()), "warn")
		a.setFooter("Ollama not reachable — mode left unchanged.", "warn")
		return
	}
	setModeOverride(nxt)
	_ = os.Setenv("AGENT_LLM_MODE", nxt)
	mode := resolveMode()
	model := modelLabel(mode)
	a.append(fmt.Sprintf("Mode set to AGENT_LLM_MODE=%s  model=%s", mode, model), "ok")
	if mode == "ollama" {
		a.append(fmt.Sprintf("Using Ollama at %s with model %s.", ollamaHost(), model), "muted")
	}
	a.setFooter(fmt.Sprintf("AGENT_LLM_MODE=%s  model=%s", mode, model), "ok")
}
...
func main() {
	...
	if !(a.footerKind == "ok" && len(a.stories) > 0) {
		a.setFooter(fmt.Sprintf("Ready (%s/%s). Arrow keys scroll. (m)ode cycles LLM.", mode, modelLabel(mode)), "info")
	}
	...
	for {
		...
		case 0:
			// ignore
		default:
			a.setFooter("Unknown key. Use menu hotkeys (t o s g m q n).", "warn")
		}
	}
}
