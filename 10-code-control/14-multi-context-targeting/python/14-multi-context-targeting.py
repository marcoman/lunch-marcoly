...
from partner import evaluate_partner, normalize_org, normalize_username  # noqa: E402
...
def init_launchdarkly() -> None:
    """Initialize the server-side SDK. Evaluation stays in this process."""
    global _ld_client
    sdk_key = (os.environ.get("LD_SDK_KEY") or "").strip()
    if not sdk_key:
        print("Warning: LD_SDK_KEY not set — partner badge stays false.", flush=True)
        return
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/flags":
            ...
            try:
                ...
                org = normalize_org((params.get("org") or [""])[0])
            except ValueError as exc:
                _json_response(self, 400, {"error": str(exc)})
                return
            _json_response(self, 200, evaluate_partner(_ld_client, username, org))
            return
        if parsed.path == "/api/bootstrap":
            _json_response(self, 200, {
                "appBanner": "14-multi-context-targeting[python]",
                "controls": api_config(),
            })
            return
...
def main() -> None:
    ...
    print("14-multi-context-targeting[python]")
