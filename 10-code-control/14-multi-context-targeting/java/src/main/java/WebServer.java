...
/**
 * Serve the multi-context grid navigator web UI on a local HTTP server.
 */
public class WebServer {
    private static final int PORT = readPort();
    public static void main(String[] args) throws IOException {
        Partner.init();
        Runtime.getRuntime().addShutdownHook(new Thread(Partner::close));

        HttpServer server = HttpServer.create(new InetSocketAddress("127.0.0.1", PORT), 0);
        server.createContext("/", WebServer::handle);
        server.start();
        System.out.println("14-multi-context-targeting[java]");
    ...
    private static void handle(HttpExchange exchange) throws IOException {
        ...
        if ("/api/bootstrap".equals(path)) {
            ...
            bootstrap.put("appBanner", "14-multi-context-targeting[java]");
    ...
    private static void handleFlags(HttpExchange exchange) throws IOException {
        ...
        try {
            String username = Partner.normalizeUsername(params.getOrDefault("username", ""));
            String org = Partner.normalizeOrg(params.getOrDefault("org", ""));
            sendJson(exchange, 200, Partner.evaluate(username, org));
