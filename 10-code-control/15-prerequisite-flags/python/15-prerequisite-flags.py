...
def init_launchdarkly() -> None:
    ...
    if not sdk_key:
        print("Warning: LD_SDK_KEY not set — flags use safe defaults.", flush=True)
        return
...
class Handler(SimpleHTTPRequestHandler):
    """Serve the UI, detailed SDK evaluations, and REST-backed controls."""
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/flags":
            ...
            try:
                ...
                payload = evaluate_prerequisite_flags(_ld_client, username)
        ...
        if parsed.path == "/api/bootstrap":
            _json_response(
                ...
                200,
                {
                    "appBanner": "15-prerequisite-flags[python]",
                    "controls": api_config(),
                },
            )
            return
        if parsed.path == "/api/flag-controls":
            try:
                ...
                _json_response(
                    self, 502, {"configured": True, "flags": [], "error": str(exc)}
                )
            return
        super().do_GET()
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            result = apply_flag_control(
                ...
                fallthrough_value=(
                    str(data["fallthrough"]).strip()
                    if data.get("fallthrough") is not None
                    else None
                ),
            )
...
def main() -> None:
    """Start the local lab server and close the SDK on shutdown."""
    init_launchdarkly()
    port = int(os.environ.get("PORT") or "8080")
    server = ThreadingHTTPServer(("127.0.0.1", port), Handler)
    print("15-prerequisite-flags[python]")
