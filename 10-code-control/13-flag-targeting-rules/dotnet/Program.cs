...
app.MapGet("/api/flags", (string? username, string? team) =>
{
    if (string.IsNullOrWhiteSpace(username))
        return Results.BadRequest(new { error = "username query parameter is required" });
    try
    {
        return Results.Json(flags.Evaluate(username.Trim(), team));
    }
...
app.MapPost("/api/flag-controls", async (HttpContext context) =>
{
    try
    {
        ...
        var fallthrough = hasFallthrough ? body["fallthrough"]?.GetValue<string>() : null;
