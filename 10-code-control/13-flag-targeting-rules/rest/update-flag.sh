...
api PATCH "/flags/${LD_PROJECT_KEY}/configure-team-label-style" \
  -H "Content-Type: application/json; domain-model=launchdarkly.semanticpatch" \
  -d "{
    \"environmentKey\":\"${LD_ENVIRONMENT_KEY}\",
    \"comment\":\"13 targeting-rules REST example: ${ACTION}\",
    \"instructions\":[{\"kind\":\"${kind}\"}]
  }" | jq ".environments.\"${LD_ENVIRONMENT_KEY}\" | {on,offVariation,fallthrough,rules}"
