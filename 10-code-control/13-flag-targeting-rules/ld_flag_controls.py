"""REST helpers for the 13 targeting-rules lab's single string flag.

Controls intentionally change only on/off and fallthrough. The three targeting
rules are provisioned by Terraform or ``rest/create-flags.sh``.
...
from team_style import FLAG_TEAM_LABEL_STYLE
...
def _variation_id(flag: dict[str, Any], value: str) -> str | None:
...
def _summarize(flag: dict[str, Any], environment: str) -> dict[str, Any]:
    ...
    options = [
        {
            "token": variation.get("value"),
            "label": variation.get("name") or variation.get("value"),
            "value": variation.get("value"),
        }
        for variation in flag.get("variations") or []
    ]
    return {
        "key": FLAG_TEAM_LABEL_STYLE,
        "name": flag.get("name") or "Configure team label style",
        ...
        "fallthroughToken": _variation_value(flag, fall_index),
        ...
        "targetingHint": (
            f"{len(env.get('rules') or [])} provisioned rules remain unchanged; "
...
def list_flag_controls() -> dict[str, Any]:
    """Fetch status and variations for the one controlled flag."""
    cfg = api_config()
    if not cfg["configured"]:
        return {
            **cfg,
    ...
    flag = _request("GET", f"/flags/{project}/{FLAG_TEAM_LABEL_STYLE}?{query}")
...
def apply_flag_control(
    ...
    if flag_key != FLAG_TEAM_LABEL_STYLE:
    ...
    if fallthrough is not None:
        wanted = str(fallthrough)
        variation_id = _variation_id(flag, wanted)
        if not variation_id:
            raise ValueError(f"No variation matching fallthrough={wanted!r}")
        instructions.append({
