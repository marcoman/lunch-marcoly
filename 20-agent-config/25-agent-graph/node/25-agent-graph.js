#!/usr/bin/env node
/**
 * 25-agent-graph.js — thin HTTP adapter for the Agent Graphs demo.
 *
 * GET  /              → index.html
...
const APP_BANNER = "25-agent-graph[node]";
const PORT = Number(process.env.PORT || 8251);
const ROOT = __dirname;
const NODE_ROLES = ["assess", "report", "questions", "good", "joke", "finalize"];
...
async function handleGenerate(req, res) {
  ...
  try {
    for await (const event of generateStream(persona, action, stories)) {
  ...
  try {
    ...
    if (req.method === "GET" && pathname === "/api/bootstrap") {
      ...
      sendJson(res, 200, {
        ...
        model: `graph:${graphKey()}`,
        graphKey: graphKey(),
        nodeKeys: Object.fromEntries(NODE_ROLES.map((r) => [r, nodeKey(r)])),
        actions: [
...
async function main() {
  ...
  server.listen(PORT, "127.0.0.1", () => {
    ...
    console.log(`LD_GRAPH_KEY=${graphKey()}`);
    console.log("Nodes: " + NODE_ROLES.map((r) => `${r}=${nodeKey(r)}`).join(", "));
