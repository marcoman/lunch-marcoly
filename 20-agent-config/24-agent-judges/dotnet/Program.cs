// 24-agent-judges — thin HTTP adapter for the Judges runtime-gate UI.
//
// =============================================================================
// HOW TO READ THIS FILE
// =============================================================================
//
// Same four jobs as 21-agent-completion-config:
//
//   1. Serve wwwroot/index.html
//   2. JSON bootstrap (personas, tickers, config/judge keys, passThreshold)
//   3. Yahoo Finance headlines
//   4. Bridge browser SSE → AgentCore.GenerateStreamAsync()
//
// The LaunchDarkly work lives in AgentCore.cs (CompletionConfig + JudgeConfig).
//
// Request map
// -----------
//   GET  /                 → index.html
//   GET  /api/bootstrap    → personas, tickers, configKey, judgeKeys, passThreshold
...
const string AppBanner = "24-agent-judges[dotnet]";
var Port = int.TryParse(Environment.GetEnvironmentVariable("PORT"), out var envPort) ? envPort : 8243;
...
Console.WriteLine($"Judges: {AgentCore.JudgeFidelityKey()}, {AgentCore.JudgeDisciplineKey()}");
Console.WriteLine($"Pass threshold: {AgentCore.PassThreshold()}");
...
static async Task HandleBootstrapAsync(HttpContext ctx)
{
    ...
    var body = new Dictionary<string, object?>
    {
        ...
        ["judgeKeys"] = new[] { AgentCore.JudgeFidelityKey(), AgentCore.JudgeDisciplineKey() },
        ["passThreshold"] = AgentCore.PassThreshold(),
    };
...
static List<Dictionary<string, object?>> ReadStoriesPayload(JsonNode? payload)
{
    ...
    // Match Node: payload.stories. Also accept tickerResults (Java/Python alias).
    JsonArray? arr = null;
