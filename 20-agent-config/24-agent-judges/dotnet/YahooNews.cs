...
public static class YahooNews
{
    ...
    /// <summary>
    /// Locate the 24-agent-judges example root (the folder containing
    ...
    public static string ExampleRoot()
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            if (File.Exists(Path.Combine(messages, "skeptic-system.txt")) ||
                File.Exists(Path.Combine(messages, "judge-source-fidelity-system.txt")))
            {
