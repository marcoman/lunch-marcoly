// 24-agent-judges[go] — console twin of the Judges runtime-gate demo (no HTTP).
//
// Same screen chrome style as 21-agent-completion-config/go, adapted for two
// personas and judge/section/rewrite stream events from agent.go.
package main
...
const (
	appBanner  = "24-agent-judges[go]"
	chromeRows = 3
	footerRows = 1
	padMax     = 4000
	menuRight  = "1=Toby  2=Charlie"
)

var menuLeft = []string{
	"(t)oby", "(c)harlie", "storie(s)", "tic(k)ers", "(g)enerate", "(q)uit",
}
...
var kindStyle = map[string]string{
	...
	"prompt":   ansiBlue,
	"response": ansiCyan,
	"section":  ansiBold + ansiYellow,
	"judge":    ansiBold + ansiMagenta,
}
...
func (a *app) appendStories() {
	if len(a.stories) == 0 {
		a.append("  (no stories loaded — press s)", "muted")
		return
	}
...
func (a *app) setPersona(index int) {
	if index < 0 || index >= len(personas) {
		return
	}
	a.personaIndex = index
	p := a.persona()
...
func (a *app) appendJudges(passed bool, threshold float64, results []judgeResult) {
	gate := "FAIL"
	kind := "error"
	if passed {
		gate = "PASS"
	...
	a.append(fmt.Sprintf("Gate: %s (threshold ≥ %.2f)", gate, threshold), kind)
	for _, r := range results {
		...
		a.append(fmt.Sprintf("  %s  score=%s  %s", r.Key, score, passLabel), lineKind)
		if r.Reasoning != "" {
			a.append("    "+r.Reasoning, "muted")
		}
		if r.Error != "" {
			a.append("    error: "+r.Error, "error")
		}
	}
}
...
func (a *app) cmdGenerate() {
	...
	if !usable {
		a.setFooter("Load stories first (press s), then g.", "warn")
		return
	}
	a.busy = true
	a.lastJudgesPassed = nil
	p := a.persona()
	...
	for event := range generateStream(p, a.stories) {
		...
		case "meta":
			...
			a.append(fmt.Sprintf("Judges: %s  threshold=%.2f", strings.Join(event.JudgeKeys, ", "), event.PassThreshold), "muted")
		...
		case "section":
			...
			a.append(fmt.Sprintf("--- %s ---", event.SectionTitle), "section")
		case "status":
		...
		case "judges":
			...
			passed := event.JudgesPassed
			a.lastJudgesPassed = &passed
			a.appendJudges(event.JudgesPassed, event.JudgeThreshold, event.JudgeResults)
		case "rewrite_meta":
			a.lastProvider = event.Provider
			a.lastModel = event.Model
			name := event.RewritePersona.Name
			if name == "" {
				name = event.Persona.Name
			}
			a.append(fmt.Sprintf("Rewrite provider: %s / %s (%s)", event.Provider, event.Model, name), "muted")
		case "error":
			if sawToken {
				a.append("", "normal")
				sawToken = false
		...
		case "done":
			msg := fmt.Sprintf("Done — report complete for %s.", p.Name)
			if a.lastJudgesPassed != nil && !*a.lastJudgesPassed {
				msg = fmt.Sprintf("Done — draft failed judges; Charlie rewrite shown for %s.", p.Name)
			}
			a.setFooter(msg, "ok")
		}
		a.render()
...
func main() {
	...
	ldHint := fmt.Sprintf("Ready (config:%s). t → s → g (Toby should fail + Charlie rewrite).", configKey())
	ldKind := "info"
	...
	for {
		...
		switch ch {
		case 't', '1':
			a.setPersona(0) // Thoughtless Toby
		case 'c', '2':
			a.setPersona(1) // Conservative Charlie
		case 's', 'o':
			a.cmdStories()
		case 'k':
			a.cmdTickers()
		...
		case 0:
			// ignore
		default:
			a.setFooter("Unknown key. Use menu hotkeys (t c s k g q).", "warn")
		}
	}
}
