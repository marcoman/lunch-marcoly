# Java web — AgentControl completion config

Same UI as [01-reference-agent](../../../01-reference-agent/java/), but **Generate AI Report** loads **model**, **system**, and **user** messages from LaunchDarkly AgentControl.

**Java has no official AI SDK.** Python/Node/Go/Ruby/.NET use an AI SDK that wraps `client.variation` for AgentControl. This example evaluates the same AI config key with the **Java server SDK** (`jsonValueVariationDetail` / `LDValue`), then interpolates `{{ stories }}` locally.
...
1. Java **21+** and Maven (or use `./mvnw`)
...
```bash
cd 20-agent-config/21-agent-completion-config/java
./mvnw -q -DskipTests package
# or: mvn -q -DskipTests package
java -jar target/21-agent-completion-config.jar
```

Open **http://127.0.0.1:8212/** (Python twin: 8210; Node twin: 8211; 01 uses 8090).
