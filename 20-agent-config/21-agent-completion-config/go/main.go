// 21-agent-completion-config[go] — terminal UI matching 01-reference-agent/go chrome/hotkeys.
//
// Same screen chrome and key bindings as 01, minus (m)ode: LaunchDarkly AgentControl
// (the completion config) owns the served model here, so there is nothing local to cycle.
// See agent.go for the LaunchDarkly insertion point (generateStream → CompletionConfig).
package main
...
const (
	appBanner  = "21-agent-completion-config[go]"
	chromeRows = 3
	footerRows = 1
	padMax     = 4000
...
var menuLeft = []string{
	"(t)ickers", "st(o)ries", "(s)tatus", "(g)enerate report", "(q)uit",
}
