# Node web — AgentControl completion config

Same UI as [01-reference-agent](../../../01-reference-agent/node/), but **Generate AI Report** loads **model**, **system**, and **user** messages from LaunchDarkly AgentControl.

Keywords: **AgentControl** · **completion config** · **AI SDK** · **message variables** (`{{ stories }}`)

| Topic | Docs |
|-------|------|
| Node AI SDK | [Node.js AI SDK reference](https://launchdarkly.com/docs/sdk/ai/node-js) |
...
1. Node.js 20 LTS+ (see repo root `.nvmrc`)
...
5. Ollama with `llama3.2:3b` (matches `Custom.llama3.2-3b`)
...
```bash
cd 20-agent-config/21-agent-completion-config/node
npm install
npm start
# or: node 21-agent-completion-config.js
```

Open **http://127.0.0.1:8211/** (Python twin uses 8210; 01 uses 8090).
