// 23-agent-tools[go] — terminal UI matching 21-agent-completion-config/go chrome/hotkeys.
//
// Same screen chrome and key bindings as 21 (no (m)ode — the served AgentControl
// variation owns provider + model). New here: a "tool" pad-line kind that
// prints each dispatched Library tool call (name / args / result) as it runs,
// so the tool loop in agent.go is visible without a separate panel. (s)tatus
// also lists the tool calls made on the last (g)enerate.
package main
...
var menuLeft = []string{
	"(t)ickers", "st(o)ries", "(s)tatus", "(g)enerate", "(q)uit",
}
...
type app struct {
	...
	lastModel     string
	lastFallback  bool
	lastToolNames []string // tool schemas attached on the served variation
	lastToolCalls []string // tool names actually dispatched on the last generate
}
...
func (a *app) cmdStatus() {
	...
	if len(a.lastToolNames) > 0 {
		a.append("Tools:    "+strings.Join(a.lastToolNames, ", "), "ok")
	} else {
		a.append("Tools:    (generate once to see attached tools)", "muted")
	}
	if len(a.lastToolCalls) > 0 {
		a.append(fmt.Sprintf("Last run: %d tool call(s) — %s", len(a.lastToolCalls), strings.Join(a.lastToolCalls, " → ")), "tool")
	} else {
		a.append("Last run: (no tool calls yet — press g)", "muted")
	}
...
// appendTool prints one dispatched tool call: name/round on its own line,
// args/result as indented JSON — the "tool trace" for the loop in agent.go.
func (a *app) appendTool(t toolTrace) {
	a.append(fmt.Sprintf("Tool #%d: %s (round %s)", t.CallIndex, t.Name, t.Round), "tool")
	if argsJSON, err := json.Marshal(t.Args); err == nil {
		a.append("  args:   "+string(argsJSON), "muted")
	}
	if resultJSON, err := json.Marshal(t.Result); err == nil {
		a.append("  result: "+string(resultJSON), "muted")
	}
}
...
func (a *app) cmdGenerate() {
	...
	for event := range generateStream(p, a.stories) {
		...
		case "meta":
			...
			a.lastToolNames = event.Tools
			a.append(fmt.Sprintf("LD: config=%s  fallback=%v", event.ConfigKey, event.Fallback), "muted")
			a.append(fmt.Sprintf("Provider: %s / %s", event.Provider, event.Model), "muted")
			if len(event.Tools) > 0 {
				a.append("Tools attached: "+strings.Join(event.Tools, ", "), "tool")
			}
		...
		case "tool":
			a.lastToolCalls = append(a.lastToolCalls, event.Tool.Name)
			a.appendTool(event.Tool)
			a.setFooter(fmt.Sprintf("Tool: %s (call #%d)…", event.Tool.Name, event.Tool.CallIndex), "busy")
		case "token":
		...
		case "metrics":
			...
			a.append(fmt.Sprintf(
				"Metrics: latency_ms=%s  ttft_ms=%s  prompt_tokens=%s  completion_tokens=%s  total_tokens=%s  finish_reason=%s  tool_calls=%d",
				fmtOptInt(m.LatencyMS), fmtOptInt(m.TTFTMS), fmtOptInt(m.PromptTokens),
				fmtOptInt(m.CompletionTokens), fmtOptInt(m.TotalTokens), dash(m.FinishReason), len(a.lastToolCalls),
			), "muted")
		case "done":
			a.setFooter(fmt.Sprintf("Done — report complete for %s.", p.Name), "ok")
		}
		a.render()
	}
	a.busy = false
...
func main() {
	...
	// generateStream() reports it as a status/error and skips the tool loop (see agent.go).
	ldHint := fmt.Sprintf("Ready (config:%s). o → g → n to compare personas.", configKey())
	ldKind := "info"
	if err := initLaunchDarkly(); err != nil {
		ldHint = fmt.Sprintf("LD init failed: %s (generate will report an error).", err)
		ldKind = "warn"
	}
	...
	for {
		...
		case 0:
			// ignore
		default:
			a.setFooter("Unknown key. Use menu hotkeys (t o s g q n).", "warn")
		}
	}
}
