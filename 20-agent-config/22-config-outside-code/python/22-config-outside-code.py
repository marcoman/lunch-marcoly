#!/usr/bin/env python3
"""
22-config-outside-code.py — thin HTTP adapter for the tracked-completion demo UI.

Same jobs as 01-reference-agent.py / 21:
  1. Serve index.html
  2. JSON bootstrap
  3. Yahoo Finance headlines
  4. Bridge browser SSE → agent_core.generate_stream()
  5. POST /api/feedback — thumbs via resumption token

LaunchDarkly work lives in agent_core.py (completion_config + track_metrics_of).
"""
...
APP_BANNER = "22-config-outside-code[python]"
# Distinct from 01 (8090) and 21 (8210–8212).
PORT = 8220


class Handler(BaseHTTPRequestHandler):
    server_version = "ConfigOutsideCodeHTTP/1.0"
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/feedback":
            persona_id = str(payload.get("personaId") or PERSONAS[0].id)
            persona = persona_by_id(persona_id) or PERSONAS[0]
            token = str(payload.get("resumptionToken") or "")
            kind = str(payload.get("kind") or "")
            try:
                body = submit_feedback(
                    persona=persona,
    ...
    def _sse_generate(self, persona, ticker_results: list) -> None:
        self.send_response(200)
