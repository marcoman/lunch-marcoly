#!/usr/bin/env bash
# LaunchDarkly: feature-flag status snapshot (32-client-identify)
# Demo-facing check: flags exist, client-side availability, on, alice/bob key rules.
...
# Usage:
#   ./get-flag-status.sh
#   ./get-flag-status.sh show-identify-move-count
  ...
  [
    {
      ...
      kind: "string",
      expectedOff: "none",
      expectedFallthroughWhenOn: "none",
    ...
    },
    {
      ...
      kind: "boolean",
      expectedOff: false,
      expectedFallthroughWhenOn: false,
...
if [[ -n "$FLAG_FILTER" ]]; then
  ...
  if [[ "$(echo "$EXPECTED_JSON" | jq 'length')" -eq 0 ]]; then
    ...
    echo "expected: enable-identify-grid-highlight | show-identify-move-count" >&2
    exit 1
  fi
fi
...
while IFS= read -r expected; do
  ...
  entry="$(jq -nc \
    ...
    | ([($e.rules // [])[]?.clauses[]?.values[]?] | unique) as $rule_vals
    | (($rule_vals | index("alice")) != null) as $has_alice
    | (($rule_vals | index("bob")) != null) as $has_bob
    | (($e.on == true) and $has_alice and $has_bob) as $rules_ok
    | {
        key: $exp.key,
        kind: $exp.kind,
        found: $found,
        ...
        targeting: {
          ...
          fallthroughMatches: ($ft_val == $exp.expectedFallthroughWhenOn),
          hasAliceRule: $has_alice,
          hasBobRule: $has_bob,
          ruleValues: $rule_vals
        },
        healthy: ($found and $client_ok and $values_ok and $rules_ok)
      }
  ')"
...
echo "$STATUS_JSON" | jq -r '
  [
      ...
      "Healthy: \(if .healthy then "yes" else "NO — check exists / client-side / on / alice+bob rules" end)",
      "",
      "Flags:",
      (.flags[] |
        ...
        + "  rules=\(if .targeting.hasAliceRule and .targeting.hasBobRule then "alice,bob" else "incomplete" end)"
...
Expected (after ./create-flags.sh):
  enable-identify-grid-highlight  on  alice→green  bob→blue  else none
  show-identify-move-count        on  alice→true   bob/else false
