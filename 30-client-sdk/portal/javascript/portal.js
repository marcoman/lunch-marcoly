 ...
 * on exit. Evaluation stays in the browser (client-side ID). This process never
...
const PORTAL_PORT = Number(process.env.PORTAL_PORT || 8300);
const APP_BANNER = "30-client-sdk[portal-javascript]";

/** cwd is each example's javascript/ so assets and node_modules resolve like a solo run. */
const CHILDREN = [
  {
    id: "31",
    ...
    script: path.join(
      ...
      "javascript",
      "31-client-evaluation.js"
    ),
    cwd: path.join(SERIES_ROOT, "31-client-evaluation", "javascript"),
    port: 8310,
  },
  {
    id: "32",
    label: "Identify",
    script: path.join(SERIES_ROOT, "32-client-identify", "javascript", "32-client-identify.js"),
    cwd: path.join(SERIES_ROOT, "32-client-identify", "javascript"),
    port: 8320,
  },
  {
    id: "33",
  ...
  if (!String(process.env.LD_CLIENT_SIDE_ID || "").trim()) {
    console.warn(
      "WARNING: LD_CLIENT_SIDE_ID is unset. Child pages cannot initialize the " +
        "JavaScript SDK until you export a client-side ID (not LD_SDK_KEY)."
    );
  }
