...
const PORTAL_PORT = Number(process.env.PORTAL_PORT || 8302);
const APP_BANNER = "30-client-sdk[portal-vue]";
const LANG = "vue";

/** cwd is each example's vue/ so Vite and node_modules resolve like a solo run. */
const CHILDREN = [
  {
    id: "31",
  ...
  if (!String(process.env.LD_CLIENT_SIDE_ID || "").trim()) {
    console.warn(
      "WARNING: LD_CLIENT_SIDE_ID is unset. Child pages cannot initialize the " +
        "Vue SDK until you export a client-side ID (not LD_SDK_KEY)."
    );
  }
