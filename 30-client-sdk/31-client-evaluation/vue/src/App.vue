...
function onSdkEvent(kind, detail) {
  ...
  console.log(`[31 evaluation][vue] ${kind}${detail ? " " + detail : ""}`);
...
watch(
  [username, closed],
  () => {
    document.body.classList.toggle("grid-active", Boolean(username.value) && !closed.value);
  },
  { immediate: true }
);
...
<template>
  ...
  <template v-else>
    <section v-if="!username" id="login-screen">
      ...
      />
      <p id="login-error" :class="loginError ? 'error' : 'error hidden'">Username is required.</p>
