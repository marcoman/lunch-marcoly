 ...
 * Membership and flag writes stay on this process — not in the page.
 */
...
const {
  ...
  applyFlagControl,
  applyMembership,
} = require("./flag-controls");

const PORT = Number.parseInt(process.env.PORT || "8330", 10);
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/flag-controls" && req.method === "POST") {
    try {
      ...
      sendJson(res, 200, await applyFlagControl(body.key, { on: body.on }));
    } catch (err) {
      sendJson(res, err.status || 500, { error: String(err.message || err) });
    }
    return;
  ...
  if (url.pathname === "/api/segment-membership" && req.method === "POST") {
    try {
      const body = await readBody(req);
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  console.log("33-synced-segments[javascript]");
