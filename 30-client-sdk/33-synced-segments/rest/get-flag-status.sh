#!/usr/bin/env bash
# Snapshot: inner-circle flag + segment (33-synced-segments).
...
DOCS_SEG="https://launchdarkly.com/docs/home/flags/synced-segments"
DOCS_API="https://launchdarkly.com/docs/api/segments/update-big-segment-targets"
...
STATUS_JSON="$(jq -nc \
  ...
  | if $verbose == 1 then . + {
      links: {
        ...
        docsSyncedSegments: $docsSeg,
        docsBigSegmentTargets: $docsApi
      }
    } else . end
')"
...
If Healthy=NO: ./create-flags.sh
Add a member:  ./add-member.sh <context-key>
Remove:        ./remove-member.sh <context-key>
Docs synced:   ${DOCS_SEG}
Docs API:      ${DOCS_API}
EOF
fi
