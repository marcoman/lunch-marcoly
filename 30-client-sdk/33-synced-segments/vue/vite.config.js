/**
 * Vite + lab REST proxy. Flag evaluation stays in the Vue SDK.
 * Token and project keys stay in this process — not in the page.
 */
import { defineConfig } from "vite";
import vue from "@vitejs/plugin-vue";
import { apiConfig, applyFlagControl, applyMembership, listFlagControls } from "./flag-controls.js";

const PORT = Number.parseInt(process.env.PORT || "8332", 10);
...
function labApiPlugin() {
  return {
    ...
    configureServer(server) {
      ...
      console.log("33-synced-segments[vue]");
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          if (pathname === "/api/segment-membership" && req.method === "POST") {
            const body = await readBody(req);
            sendJson(res, 200, await applyMembership(body.contextKey, body.action));
            return;
          }
