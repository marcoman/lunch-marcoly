...
const ROWS = ["t", "m", "b"];
const COLS = ["l", "m", "r"];
const BANNER = "34-synced-segments-twilio[react]";
...
function App() {
  ...
  const [segmentWriteKey, setSegmentWriteKey] = useState(null);
  ...
  const onSdkEvent = useCallback((kind, detail) => {
    ...
    console.log(`[34 synced-segments-twilio][react] ${kind}${detail ? " " + detail : ""}`);
    setSdkCallLog((prev) => {
  ...
  useEffect(() => {
    fetch("/api/config", { cache: "no-store" })
      ...
      .then((cfg) => {
        ...
        setSegmentWriteKey(cfg.segmentWriteKey || null);
        refreshControls();
      })
...
function LoggedInUI({
  ...
  useEffect(() => {
    if (username && segmentWriteKey) {
      segmentIdentify(segmentWriteKey, username).catch(() => {});
    }
  }, [username, segmentWriteKey]);
  ...
  async function identifyUser(name) {
    ...
    try {
      ...
      onContextKey(next);
      if (segmentWriteKey) await segmentIdentify(segmentWriteKey, next);
      gridRef.current?.focus();
    } finally {
  ...
  async function postMembership(action) {
    ...
    try {
      if (action === "add") {
        await segmentJoinInnerCircle(segmentWriteKey, username);
        setMembershipMsg(
          `Sent identify + track “Joined Inner Circle” for ${username}. ${SEGMENT_SYNC_HINT}`
        );
      } else {
        await segmentLeaveInnerCircle(segmentWriteKey, username);
        setMembershipMsg(
          `Sent identify + track “Left Inner Circle” for ${username}. ${SEGMENT_SYNC_HINT}`
        );
      }
      await onRefresh();
    } catch (err) {
  ...
  if (seg.found) {
    ...
    segmentMeta = `${seg.key} not found. Twilio creates it on first Audiences sync; set LD_TWILIO_SEGMENT_KEY.`;
  }
